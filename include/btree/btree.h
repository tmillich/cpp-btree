...
static bool btree_compare_keys(const Compare &comp, const Key &x,
                               const Key &y) {
  using key_comparer =
      btree_key_comparer<Key, Compare, btree_is_key_compare_to<Compare>::value>;
...
struct btree_common_params {
  ...
  using node_count_type = typename if_<(kNodeValueSpace / ValueSize) >= 256,
...
struct btree_linear_search_compare_to {
  ...
  static int upper_bound(const K &k, const N &n, CompareTo comp) {
    using upper_compare =
        btree_upper_bound_adapter<K, btree_key_comparer<K, CompareTo, true> >;
    return n.linear_search_plain_compare(k, 0, n.count(), upper_compare(comp));
  }
};
...
class btree_node {
 ...
 public:
  ...
  reference value(int i) { return fields_.values[i]; }
  const_reference value(int i) const { return fields_.values[i]; }
 ...
 private:
  ...
  void value_init(int i, args_type &&... args) {
    new (&fields_.mutable_values[i])
        mutable_value_type(std::forward<args_type>(args)...);
  }
  void value_destroy(int i) { fields_.mutable_values[i].~mutable_value_type(); }
...
class btree : public Params::key_compare {
 ...
 public:
  ...
  // Inserts a new element into the btree constructed in-place with the given
  // args
 ...
 private:
  ...
  static_assert(sizeof(key_compare_checker(key_compare_helper()(key_type(),
                                                                key_type()))) ==
                    sizeof(big_),
...
template <typename P>
template <typename... value_args_type>
inline void btree_node<P>::insert_value(int i,
                                        value_args_type &&... value_args) {
  assert(i <= count());
...
template <typename P>
btree<P>::btree(const allocator_type &alloc) : root_(alloc, nullptr) {}
...
template <typename P>
template <typename... args_type>
std::pair<typename btree<P>::iterator, bool> btree<P>::emplace_unique(
    const key_type &key, args_type &&... args) {
  if (empty()) {
    *mutable_root() = new_leaf_root_node(1);
  }
  ...
  return std::make_pair(
      internal_insert(iter, key, std::forward<args_type>(args)...), true);
}

template <typename P>
template <typename... args_type>
inline typename btree<P>::iterator btree<P>::emplace_unique_hint(
    iterator position, const key_type &key, args_type &&... args) {
  if (!empty()) {
    if (position == end() || compare_keys(key, position.key())) {
      iterator prev = position;
      if (position == begin() || compare_keys((--prev).key(), key)) {
...
template <typename P>
template <typename... args_type>
typename btree<P>::iterator btree<P>::emplace_multi(const key_type &key,
                                                    args_type &&... value) {
  if (empty()) {
    *mutable_root() = new_leaf_root_node(1);
  }
...
template <typename P>
template <typename... args_type>
typename btree<P>::iterator btree<P>::emplace_multi_hint(
    iterator position, const key_type &key, args_type &&... value) {
  if (!empty()) {
    if (position == end() || !compare_keys(position.key(), key)) {
      iterator prev = position;
      if (position == begin() || !compare_keys(key, (--prev).key())) {
        // prev.key() <= key <= position.key()
        return internal_insert(position, key,
                               std::forward<args_type>(value)...);
      }
    } else {
      iterator next = position;
      ++next;
...
inline typename btree<P>::iterator btree<P>::internal_insert(
    iterator iter, value_args_type &&... value_args) {
  if (!iter.node->leaf()) {
  ...
  iter.node->insert_value(iter.position,
                          std::forward<value_args_type>(value_args)...);
  return iter;
}
